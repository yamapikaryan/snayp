...
//        ];
        ...
        if (!empty($isEis)) {
            ...
            // Название аукциона
            $auctionObject = $html->find('.h3 .s2', 0);
            $auction['auctionObject'] = $auctionObject->innertext;

            // Номер аукциона
            preg_match('/tender-(\d+)\//i', $this->url, $tmp);
            $auction['auctionNumber'] = $tmp[1];

            // Запрос или аукцион
            $auctionObjectRaw = $html->find('h1[itemprop=headline]', 0);
            $objectText = mb_strtolower($auctionObjectRaw->innertext);
            if (strpos($objectText, 'запрос') !== false) {
            ...
            // Цена
            $auctionPrice = $html->find('tr[id=trade-info-lot-price]', 0);
